  ...
  async function search(keyword, page = 1, perPage = 20) {
    try {
      ...
      if (alquranCloudSearch?.data?.matches) {
        console.log('Using alquran.cloud search:', alquranCloudSearch);
        return {
          data: {
      ...
      console.log('Results array:', results, 'total:', totalResults);
      ...
      if (results.length > 0) {
        const matches = results.map(result => {
          ...
          // Debug: log translations available
          console.log('Result translations:', result.translations);

          // Find Al-Hilali translation (resource_id 20)
          const hilaliTrans = result.translations?.find(t => t.resource_id === 20);
          console.log('Found Hilali:', hilaliTrans);
      ...
      return { data: { matches: 0, matches: [], total: 0, currentPage: 1, hasMore: false } };
    } catch (error) {
